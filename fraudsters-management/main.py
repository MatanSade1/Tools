...
Converts the Jupyter notebook fraud detection process into a production Cloud Run service.
Executes 4 steps:
1. Calculate potential_fraudsters table
2. Calculate offer_wall_progression_cheaters table
3. Update fraudsters table in BigQuery
4. Update Mixpanel cohort with fraudster markers
"""
...
CONSISTENT_COHORT_MARKER = "fraudster_cohort_active_v8"
PROJECT_ID = "yotam-395120"
DATASET_ID = "peerplay"

# Setup logging
logging.basicConfig(level=logging.INFO)
logger = logging.getLogger('FraudstersManagement')
...
def step_1_calculate_potential_fraudsters(client: bigquery.Client, run_id: str) -> Dict[str, Any]:
    """Step 1: Calculate the potential_fraudsters table"""
    logger.info("Starting Step 1: Calculate potential_fraudsters table")
    ...
    CREATE OR REPLACE TABLE `yotam-395120.peerplay.potential_fraudsters` AS (
    ...
    try:
        ...
        logger.info("✓ Step 1 completed: potential_fraudsters table created")
...
def step_2_calculate_offerwall_cheaters(client: bigquery.Client, run_id: str) -> Dict[str, Any]:
    """Step 2: Calculate the offerwalls cheaters table"""
    logger.info("Starting Step 2: Calculate offer_wall_progression_cheaters table")
    log_step(client, run_id, "2", is_start=True)
    
    query = """
    -- Insert the table results to offer_wall_progression_cheaters table
    CREATE OR REPLACE TABLE `yotam-395120.peerplay.offer_wall_progression_cheaters` AS
    SELECT * FROM `yotam-395120.peerplay.potential_fraudsters`
...
def step_3_update_fraudsters_table(client: bigquery.Client, run_id: str) -> Dict[str, Any]:
    """Step 3: Update fraudsters table in BigQuery"""
    logger.info("Starting Step 3: Update fraudsters table")
    ...
    TRUNCATE TABLE `yotam-395120.peerplay.fraudsters_temp`;

    -- Insert from potential_fraudsters (automated fraud detection)
    INSERT INTO `yotam-395120.peerplay.fraudsters_temp` (
      distinct_id,
    ...
    WHERE manual_identification_fraud_purchase_flag=1
      AND distinct_id NOT IN (
        SELECT distinct_id
        FROM `yotam-395120.peerplay.potential_fraudsters`
      );

    -- Handle overlapping users (exist in both potential_fraudsters AND have manual fraud purchases)
    -- Update their manual flag to 1 while keeping their automated flags
    UPDATE `yotam-395120.peerplay.fraudsters_temp`
    ...
    -- Final step: replace the content in the fraudsters table
    CREATE OR REPLACE TABLE yotam-395120.peerplay.fraudsters
    AS
    SELECT * FROM yotam-395120.peerplay.fraudsters_temp;
    """
    ...
    try:
        ...
        query_job = client.query(query)
        query_job.result()  # Wait for completion
        logger.info("✓ Step 3 completed: fraudsters table updated")
...
def get_distinct_ids_from_bigquery(client: bigquery.Client) -> List[str]:
    """Fetch distinct IDs from BigQuery fraudsters_temp table"""
    logger.info(f"Fetching distinct IDs from {PROJECT_ID}.{DATASET_ID}.fraudsters_temp...")
    
    query = f"""
    SELECT DISTINCT distinct_id
    FROM `{PROJECT_ID}.{DATASET_ID}.fraudsters_temp`
...
def update_user_profiles_with_marker(distinct_ids: List[str]) -> tuple:
    """
    Update Mixpanel user profiles with a CONSISTENT cohort marker property
    """
    logger.info(f"Updating {len(distinct_ids)} user profiles with CONSISTENT cohort marker...")
...
def step_4_update_mixpanel_cohort(client: bigquery.Client, run_id: str) -> Dict[str, Any]:
    """Step 4: Update Mixpanel cohort with fraudster markers"""
    logger.info("Starting Step 4: Update Mixpanel cohort")
    ...
    try:
        ...
        # Update profiles with consistent marker
        logger.info(f"Using CONSISTENT cohort marker: '{CONSISTENT_COHORT_MARKER}'")
        ...
        log_step(client, run_id, "4", is_start=False)
        return {"success": True, "step": 4, "updated_count": successful_updates}
    except Exception as e:
...
def run_fraudsters_management():
    ...
    logger.info("=" * 60)
    logger.info("FRAUDSTERS MANAGEMENT PROCESS - STARTING")
    logger.info("=" * 60)
    ...
    logger.info("=" * 60)
    logger.info(f"FRAUDSTERS MANAGEMENT PROCESS - {'SUCCESS' if results['success'] else 'COMPLETED WITH ERRORS'}")
    logger.info("=" * 60)
...
def handle_request():
    ...
    logger.info("Fraudsters Management Cloud Run service invoked")
